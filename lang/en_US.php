<?php 
	$lang['friendlyname'] = 'Dev4 Auctions';
	$lang['help'] = 'Some usefull text that will explaine nothing';
	$lang['changelog'] = 'nothing to see, all changes are bugs!';
	$lang['admindescription'] = 'aah administrators, Hide the bad code under the carpet!!';
	$lang['title_settings'] = 'Settings';
	$lang['title_general'] = 'Auctions';
	$lang['title_products'] = 'Products';

	$lang['title_name'] = 'Name Auction: ';
	$lang['title_product'] = 'Auction Product';
	$lang['title_description'] = 'Auction description';
	$lang['submit'] = 'Submit';
	$lang['addauction']  = 'Create new Auction';

	$lang['edit'] = 'Edit';
	$lang['delete'] = 'Remove';
?>