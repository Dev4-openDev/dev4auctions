...
if (isset($params['product_id']) && $params['product_id'] != -1)
   ...
   $query = 'UPDATE '.cms_db_prefix().'module_dev4auctions_products set name=?, description=?, productimage=? where product_id= ?';
   $result = $db->Execute($query,array($params['name'],$params['description'],$params['productimage'], $params['product_id']));
   ...
   $query = 'INSERT INTO '.cms_db_prefix().'module_dev4auctions_products (product_id, name, description, productimage) VALUES (?,?,?,?)';
   $result = $db->Execute($query,array($sid,$params['name'],$params['description'], $params['productimage']));
